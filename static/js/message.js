;(function (global, $) {
    ...
    function generateCss(map) {
        ...
        let st = createEle('style', {attrs: {id: 'message_style'}, props: {innerHTML: str}});
    ...
    function updateIECss() {
        if (isIE()) {
            let str = '.toast-icon--success:after{left:-5px;}.toast-icon--warning:after{left:-8px;}.toast-icon--error:after{left:-6px;}';
    ...
    $.message = function (options) {
        if (options) {
            ...
            var ele = createEle('div', {
                ...
                }, props: {innerHTML: options.message}
            }), options.showClose ? createEle('i', {
                attrs: {
                    class: "toast-message__closeBtn"
                }, props: {onclick: closeEle(options.onClose)}
            }) : null]);
            let currentEle = document.querySelector('.toast-message');
            if (currentEle) {
